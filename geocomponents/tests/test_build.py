...
def test_scalar_outward_identifier_server_managed_uses_referenced_property_value():
    ds = _make_dataset(
        ...
        outward_identifier_path="identifikasjon.lokalid",
        server_managed_paths={
            "eksternpeker": "outward_identifier",
            "identifikasjon.versjonid": "timestamp_iso",
        },
    )
    plan = build_schema_plan(ds)
    scalar_col = next(
        c for c in plan.collections[0].table.columns if c.name == "eksternpeker"
    )
    assert (
        scalar_col.server_write_expr
        == "(feature->'properties'->'identifikasjon'->>'lokalid')"
    )

    identifikasjon_col = next(
        c for c in plan.collections[0].table.columns if c.name == "identifikasjon"
    )
    assert ("versjonid", "now()::text") in identifikasjon_col.write_inject
    assert "versjonid" in identifikasjon_col.strip_keys
