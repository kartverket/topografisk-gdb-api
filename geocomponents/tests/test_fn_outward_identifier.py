...
def oid_db(db):
    """Apply the test_oid schema; dropped fresh at module start."""
    conn = psycopg.connect(db)
    with conn.transaction():
        conn.execute(f"drop schema if exists {_DS} cascade")
...
def _feature(*, fid: str | None = None, lokalid: str | None = None) -> dict:
...
def _oi_of(feature: dict) -> str | None:
    return feature["properties"]["identifikasjon"].get("lokalid")
...
class CreateCase:
    ...
    lokalid: str | None  # value for identifikasjon.lokalid
...
def test_create_accepted(oid_conn, case: CreateCase):
    ...
    new_id = _create(oid_conn, _feature(fid=case.fid, lokalid=case.lokalid))
...
def test_create_rejected(oid_conn, case: CreateCase):
    ...
    with pytest.raises(psycopg.Error) as exc:
        _create(oid_conn, _feature(fid=case.fid, lokalid=case.lokalid))
...
def test_upsert_accepted(oid_conn, case: CreateCase):
    ...
    new_id = _upsert(oid_conn, _feature(fid=case.fid, lokalid=case.lokalid))
    if case.expected_id is not None:
        assert new_id == case.expected_id
    got = _fetch(oid_conn, new_id)
    assert got is not None
...
def test_upsert_rejected(oid_conn, case: CreateCase):
    ...
    with pytest.raises(psycopg.Error) as exc:
        _upsert(oid_conn, _feature(fid=case.fid, lokalid=case.lokalid))
    assert exc.value.sqlstate == case.expected_sqlstate
...
def test_update_with_different_oi_is_rejected(oid_conn):
    ...
    with pytest.raises(psycopg.Error) as exc:
        _update(oid_conn, fid, _feature(lokalid=_u(0xE1)))
...
def test_update_with_same_oi_is_accepted(oid_conn):
    ...
    fid = _u(0xF0)
    _create(oid_conn, _feature(fid=fid))
    assert _update(oid_conn, fid, _feature(lokalid=fid)) is True
...
def test_replace_with_different_oi_is_rejected(oid_conn):
    ...
    _create(oid_conn, _feature(fid=fid))
    with pytest.raises(psycopg.Error) as exc:
        _replace(oid_conn, fid, _feature(lokalid=_u(0x81)))
    assert exc.value.sqlstate == "P0001"
...
def test_replace_with_same_oi_is_accepted(oid_conn):
    ...
    fid = _u(0x90)
    _create(oid_conn, _feature(fid=fid))
    assert _replace(oid_conn, fid, _feature(lokalid=fid)) is True
...
def test_plain_collection_without_oi_is_unaffected(oid_conn):
    ...
    assert got is not None
    assert got["id"] == new_id
    assert "identifikasjon" not in got["properties"]
