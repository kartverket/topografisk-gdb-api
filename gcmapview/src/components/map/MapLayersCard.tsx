  ...
  return (
    <li>
      <button
        ...
        title={
          visible
            ? `Hide ${MAP_LAYER_LABELS[layerId]}`
            : `Show ${MAP_LAYER_LABELS[layerId]}`
        }
        onClick={onToggle}
      >
        {layerId === 'parcels' ? (
          <span
            className={cn(layerSwatchClassName, 'h-2.5 w-4 bg-[#ffc040]')}
          />
          ...
          <span
            className={cn(
              layerSwatchClassName,
              isLine ? 'h-1 w-4' : 'h-2.5 w-4',
            )}
            style={{
              background:
        ...
        <ul className="m-0 space-y-1 p-0 text-sm">
          {MAP_LAYER_IDS.map((layerId) => (
            <LayerToggleRow
              ...
              readOnly={
                layerId === 'platformEdges' || layerId === 'trackCentres'
              }
