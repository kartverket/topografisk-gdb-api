.PHONY: lock install docker-up docker-down docker-delete-db-volumes docker-trivy-scan frontend-install frontend-build frontend-run frontend-lint frontend-format gcapi-lock gcapi-install gcapi-test gcapi-run gcimport-lock gcimport-install gcimport-test gcimport-run gccore-lock gccore-install gccore-test gccore-run gcjobs-lock gcjobs-install gcjobs-test gcjobs-run

DOCKER_COMPOSE := $(shell command -v docker-compose >/dev/null 2>&1 && echo docker-compose || echo 'docker compose')
DOCKER_SERVICES := geocomponents gcapi gcimport gccore gcjobs gcmapview
...
gcapi-test:
	cd gcapi && uv run pytest

gcapi-run:
	uv run --project gcapi gcapi serve --port 8004
